...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, `parse_block_ref`, token display helpers, grammar routines, parser diagnostic span helpers, typed AST construction for type/pattern/atom/postfix/operator/assignment/simple-statement/control-flow/block/param-field/item/kernel-root paths, typed block consumers, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, `AstTypeRef`, `AstExprRef`, `AstStmtRef`, `AstItemRef`, `AstBlockRef`, and `AstPatternRef`. |
