...
| `modules/ast_report.bkr` | `build_ast_report_comment`, `self_host_ast_json`, complete-tree `ast_report_*` serializers that consume typed AST refs, typed AST collection refs, semantic block accessors, and semantic expression accessors, plus summary/root helpers that consume semantic top-level item/kernel accessors. |
