...
| `modules/kind_model.bkr` | `ast_node_kind_name`, `ast_type_kind_name`, `ast_pattern_kind_name`, `ast_category_name`, `ast_node_category_kind`, and pure `ast_is_*_kind` predicates, with node-kind helpers typed as `NodeKind`. |
| `modules/ast.bkr` | `Ast*Ref` wrapper structs, typed AST collection ref structs, typed AST node/pattern builder refs, typed AST constructor refs, NodeKind-typed constructor/kind readers, typed AST category-to-node upcasts, typed AST node-to-item/type/expression/block/statement upcasts, typed AST node category refs, typed AST node-list append refs, typed item/param/field/kernel accessors, typed type detail accessors, typed statement/block accessors, typed expression accessors, typed expression span accessors, typed pattern value/kind/span accessors, raw `ast_*` constructor adapters/accessors, AST node/type/expression/pattern handle helpers, semantic child-handle accessors, explicit optional-field handle accessors, typed AST field conversion helpers, optional AST handle helpers, source-span helpers, typed collection bridge helpers, block iteration helpers, node-level category helpers, and predicates for the current AST vector/type/pattern layout. |
...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, `parser_*_to_compat_*` raw exit helpers, `parse_match_expr_ref`, `parse_atom_ref`, `parse_postfix_ref`, `parse_mul_ref`, `parse_add_ref`, `parse_shift_ref`, `parse_bit_and_ref`, `parse_bit_xor_ref`, `parse_bit_or_ref`, `parse_comparison_ref`, `parse_and_ref`, `parse_or_ref`, `parse_ternary_ref`, `parse_match_pattern_ref`, `parse_type_ref`, `parse_expr_ref`, `parse_block_ref`, `parse_params_ref`, `parse_*_stmt_ref`, `parse_*_ref` item helpers, token display helpers, grammar routines, parser diagnostic span helpers, typed AST construction for type/pattern/atom/postfix/operator/assignment/simple-statement/control-flow/block/param-field/item/kernel-root paths, typed node-list child appends, typed atom/postfix/operator/expression-ladder/pattern/type/expression/statement/block consumers, direct typed type/expression parser ownership, typed expression span reads, named compatibility conversions, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, `AstTypeRef`, `AstExprRef`, `AstStmtRef`, `AstItemRef`, `AstBlockRef`, and `AstPatternRef`. |
...
| `modules/c_codegen.bkr` | `gen_c_program`, C escaping/name helpers, and typed-ref type inference/emission helpers that consume typed AST collection refs, typed top-level item/kernel accessors, semantic statement/block accessors, semantic expression accessors, and `CgenStateRef`. |
...
| `modules/ast_report.bkr` | `build_ast_report_comment`, `self_host_ast_json`, complete-tree `ast_report_*` serializers that consume typed AST refs, typed AST collection refs, typed declaration accessors, semantic block accessors, and semantic expression accessors, plus summary/root helpers that consume semantic top-level item/kernel accessors. |
| `modules/symbol_table.bkr` | `build_symbol_table_report_comment`, `self_host_symbol_table_json`, and `symbol_table_*` declaration/reference walkers that consume typed AST refs, typed AST collection refs, typed top-level item/kernel accessors, semantic statement/block accessors, and semantic expression accessors. |
| `modules/type_graph.bkr` | `build_type_graph_report_comment`, `self_host_type_graph_json`, `type_graph_*` JSON helpers that consume typed AST refs, typed AST collection refs, semantic top-level item/kernel accessors, semantic statement/block accessors, and `CgenStateRef` bootstrap type-state helpers used by typecheck. |
| `modules/resolver.bkr` | `build_resolver_report_comment`, `self_host_resolver_json`, and bootstrap resolver diagnostics helpers that consume typed AST refs, typed AST collection refs, semantic top-level item/kernel accessors, semantic statement/block accessors, and semantic expression accessors. |
| `modules/typecheck.bkr` | `build_typecheck_report_comment`, `self_host_typecheck_json`, and bootstrap typecheck diagnostics helpers that consume typed AST refs, typed AST collection refs, semantic top-level item/kernel accessors, semantic statement/block accessors, semantic expression accessors, and `CgenStateRef` type-state. |
