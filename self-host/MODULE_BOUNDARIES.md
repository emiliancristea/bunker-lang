...
| 7 | `modules/parser.bkr` | `PARSER` | Build raw `Vec<i64>` AST nodes from lexer tables through `ParserStateRef`, parser-side typed span refs, and AST helpers. |
...
| `modules/ast.bkr` | `Ast*Ref` wrapper structs, typed AST collection ref structs, typed AST node/pattern builder refs, `ast_*` constructors/accessors, AST node/type/expression/pattern handle helpers, semantic child-handle accessors, explicit optional-field handle accessors, typed AST field conversion helpers, optional AST handle helpers, source-span helpers, typed collection bridge helpers, block iteration helpers, node-level category helpers, and predicates for the current AST vector/type/pattern layout. |
...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, token display helpers, grammar routines, parser diagnostic span helpers, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, and `AstPatternRef`. |
