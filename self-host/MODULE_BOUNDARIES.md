...
| 7 | `modules/parser.bkr` | `PARSER` | Build raw `Vec<i64>` AST nodes from lexer tables through `ParserStateRef`, parser-side typed category span refs, and AST helpers. |
...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, token display helpers, grammar routines, parser diagnostic span helpers, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, `AstTypeRef`, `AstExprRef`, `AstStmtRef`, `AstItemRef`, `AstBlockRef`, and `AstPatternRef`. |
