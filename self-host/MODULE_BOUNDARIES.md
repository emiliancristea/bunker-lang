...
| 7 | `modules/parser.bkr` | `PARSER` | Build AST nodes from lexer tables through `ParserStateRef`, typed primary/operator/simple-statement/control-flow/param-field construction refs, parser-side typed category span refs, and raw compatibility adapters while parser signatures remain bootstrap-compatible. Item and kernel root construction remain on bootstrap raw adapters pending stage2-safe typed wrapper validation. |
