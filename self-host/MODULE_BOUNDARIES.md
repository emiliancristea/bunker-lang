...
| `modules/ast.bkr` | `Ast*Ref` wrapper structs, typed AST collection ref structs, `ast_*` constructors/accessors, AST node/type/expression/pattern handle helpers, semantic child-handle accessors, explicit optional-field handle accessors, typed AST field conversion helpers, optional AST handle helpers, source-span helpers, typed collection bridge helpers, block iteration helpers, node-level category helpers, and predicates for the current AST vector/type/pattern layout. |
