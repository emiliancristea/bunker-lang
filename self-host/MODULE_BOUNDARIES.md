...
| 7 | `modules/parser.bkr` | `PARSER` | Build AST nodes from lexer tables through `ParserStateRef`, typed primary/operator/simple-statement/control-flow/block/param-field/item/kernel-root construction refs, typed type/expression/statement/block consumers, parser-side typed category span refs, and raw compatibility adapters while parser signatures remain bootstrap-compatible. |
...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, `parse_type_ref`, `parse_expr_ref`, `parse_block_ref`, `parse_*_stmt_ref`, token display helpers, grammar routines, parser diagnostic span helpers, typed AST construction for type/pattern/atom/postfix/operator/assignment/simple-statement/control-flow/block/param-field/item/kernel-root paths, typed type/expression/statement/block consumers, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, `AstTypeRef`, `AstExprRef`, `AstStmtRef`, `AstItemRef`, `AstBlockRef`, and `AstPatternRef`. |
