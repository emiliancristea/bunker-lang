...
| 7 | `modules/parser.bkr` | `PARSER` | Build AST nodes from lexer tables through `ParserStateRef`, typed primary-construction refs, parser-side typed category span refs, and raw compatibility adapters while parser signatures remain bootstrap-compatible. |
...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, token display helpers, grammar routines, parser diagnostic span helpers, typed primary AST construction for type/pattern/atom paths, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, `AstTypeRef`, `AstExprRef`, `AstStmtRef`, `AstItemRef`, `AstBlockRef`, and `AstPatternRef`. |
