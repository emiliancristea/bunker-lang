...
| `modules/parser.bkr` | `parse_kernel`, `parse_kernel_ref`, `parse_match_expr_ref`, `parse_atom_ref`, `parse_postfix_ref`, `parse_mul_ref`, `parse_add_ref`, `parse_match_pattern_ref`, `parse_type_ref`, `parse_expr_ref`, `parse_block_ref`, `parse_*_stmt_ref`, token display helpers, grammar routines, parser diagnostic span helpers, typed AST construction for type/pattern/atom/postfix/operator/assignment/simple-statement/control-flow/block/param-field/item/kernel-root paths, typed atom/postfix/operator/pattern/type/expression/statement/block consumers, and parser-side AST span attachment through `ParserStateRef`, `AstNodeRef`, `AstTypeRef`, `AstExprRef`, `AstStmtRef`, `AstItemRef`, `AstBlockRef`, and `AstPatternRef`. |
