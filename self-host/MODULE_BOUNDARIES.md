...
| 7 | `modules/parser.bkr` | `PARSER` | Build AST nodes from lexer tables through `ParserStateRef`, typed primary/operator/simple-statement/control-flow construction refs, parser-side typed category span refs, and raw compatibility adapters while parser signatures remain bootstrap-compatible. Param, field, item, and kernel root construction remain on bootstrap raw adapters pending stage2-safe typed wrapper validation. |
