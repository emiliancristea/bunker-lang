...
| `modules/type_graph.bkr` | `build_type_graph_report_comment`, `self_host_type_graph_json`, `type_graph_*` JSON helpers that consume typed AST refs, typed AST collection refs, semantic top-level item/kernel accessors, and `CgenStateRef` bootstrap type-state helpers used by typecheck. |
