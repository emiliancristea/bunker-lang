...
| `modules/c_codegen.bkr` | `gen_c_program`, C escaping/name helpers, and typed-ref type inference/emission helpers that consume typed AST collection refs, typed top-level item/kernel accessors, semantic statement/block accessors, and `CgenStateRef`. |
