...
| Q-064 | DONE | Route parser statement and item construction through refs. | `modules/parser.bkr` now builds lets, returns, break/continue, if/for/while/loop, params, functions, constants, structs, fields, and kernel roots through typed AST constructors and span helpers; CI now rejects direct parser span mutation. |
...
| 2026-05-02 | Routed parser statement and item construction through typed refs. | Parser statement, param, item, field, and kernel-root construction now uses typed AST constructors and typed parser span helpers; direct parser `ast_set_span` use is CI-blocked. |
