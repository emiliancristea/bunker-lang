...
| T-009 | PARTIAL | P0 | User-defined enums/sum types. | Rust and self-host compilers parse unit enums, `Enum.Variant` construction/match, single-type payload declarations, `Enum.Variant(expr)` construction packed as `(payload << 8) \| tag`, and `Enum.Variant(x)` match bindings; Rust and self-host exhaustiveness are enforced for unit enums; final gate requires multi-field payloads and richer ADT lowering. |
