...
| 2026-05-02 | Added temp-dir self-host phase trace capture. | `self-host-compile` can now enable a Bunker-written phase trace with `BUNKER_SELF_HOST_TRACE=1`; CI prints the temp trace file when stage1 full `bkrc` compilation fails, avoiding local builds while diagnosing the remaining hang. |
