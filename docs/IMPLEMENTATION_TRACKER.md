...
| 2026-05-02 | Routed parser param/field construction through typed refs. | Parser function params and struct fields now use typed node constructors and typed span helpers; item roots remain raw pending the next stage2-safe slice. |
| 2026-05-02 | Routed parser item construction through typed refs. | Parser function, constant, and struct item roots now use typed item constructors and typed parser span helpers; kernel root remains raw pending isolated validation. |
| 2026-05-02 | Isolated parser kernel root typed conversion and kept it blocked. | PR #88 proved kernel root conversion still times out in GitHub self-host smoke even with explicit `AstNodeListRef`, `AstStringListRef`, and `AstNodeRef` locals; item roots, params/fields, control-flow, simple statements, operators, and primaries are green. |
