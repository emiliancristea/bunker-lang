...
| Q-066 | BLOCKED | Route parser param/field construction through refs. | PR checks passed, but the post-merge `main` self-host smoke timed out during full `bkrc` self-compilation after this routing; keep params/fields on raw bootstrap adapters until the stage2 typed-wrapper instability is diagnosed. |
| Q-067 | BLOCKED | Route parser item construction through refs. | Item-root routing depends on stable parser param/field/list wrapper behavior; keep function, const, and struct item roots on raw bootstrap adapters until Q-066 is stage2-safe. |
