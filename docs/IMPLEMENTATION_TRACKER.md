...
| CF-003 | PARTIAL | P0 | AST definitions in Bunker. | AST construction, parser/codegen reads, AST collection refs/access, optional AST handles, node/type/expression/pattern handle conversion, semantic child-handle access, explicit handle-named optional field access, read-only AST wrapper structs, C-codegen typed-ref consumption, AST-report typed-ref consumption, symbol-table typed-ref consumption, type-graph typed-ref consumption, resolver typed-ref consumption, typecheck typed-ref consumption, internal field conversion, AST kind/category/span reasoning, and codegen state typed-ref consumption are centralized in Bunker helpers; final gate requires structs/enums instead of raw `Vec<i64>` tags. |
