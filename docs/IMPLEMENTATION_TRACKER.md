...
| T-009 | PARTIAL | P0 | User-defined enums/sum types. | Rust and self-host compilers parse unit enums, `Enum.Variant` construction/match, and single-type variant payload declarations; Rust and self-host exhaustiveness are enforced for unit enums; final gate requires payload construction, match bindings, and codegen. |
