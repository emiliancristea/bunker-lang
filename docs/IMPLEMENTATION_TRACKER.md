...
| T-009 | PARTIAL | P0 | User-defined enums/sum types. | Rust and self-host compilers parse unit enums, `Enum.Variant` construction/match, single-type payload declarations, and `Enum.Variant(expr)` construction packed as `(payload << 8) \| tag`; Rust and self-host exhaustiveness are enforced for unit enums; final gate requires payload match bindings. |
