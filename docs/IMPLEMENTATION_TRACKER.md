...
| Q-060 | DONE | Add typed AST constructor refs. | `modules/ast.bkr` now exposes typed-returning constructors for types, expressions, statements, items, blocks, and kernel roots, with raw constructors reduced to compatibility adapters and guarded in CI. |
| Q-061 | DONE | Add parser category span refs. | `modules/ast.bkr` now upcasts typed type/expression/statement/item/block refs to `AstNodeRef`, and `modules/parser.bkr` exposes parser span helpers for each category with CI guard coverage. |
| Q-062 | DONE | Route parser primary construction through refs. | `modules/parser.bkr` now builds type nodes, match patterns, match expressions, literals, arrays, struct literals, calls, identifiers, unary expressions, and fallback atoms through typed AST constructors and category span helpers, with CI guard coverage. |
| Q-063 | DONE | Route parser operator construction through refs. | `modules/parser.bkr` now builds postfix index/field access, binary precedence expressions, ternary expressions, assignments, and expression statements through typed AST constructors plus explicit typed span helpers, with CI guard coverage. |
| Q-064 | DONE | Route parser simple-statement construction through refs. | `modules/parser.bkr` now builds lets, returns, break, and continue through typed statement constructors and category span helpers, with CI guard coverage. |
| Q-065 | DONE | Route parser control-flow construction through refs. | `modules/parser.bkr` now builds if, for, while, and loop statements through typed statement constructors and typed span helpers while leaving block append/item construction on bootstrap compatibility paths. |
| Q-066 | DONE | Route parser param/field construction through refs. | `modules/parser.bkr` now builds function params and struct fields through typed node constructors and explicit typed span helpers while leaving item roots on bootstrap compatibility paths. |
| Q-067 | DONE | Route parser item construction through refs. | `modules/parser.bkr` builds function, constant, and struct item roots through typed item constructors and typed span helpers, setting up the final kernel-root parser slice. |
| Q-068 | DONE | Route parser kernel construction through refs. | `modules/parser.bkr` now builds the kernel root through explicit `AstNodeListRef`, `AstStringListRef`, and `AstNodeRef` locals, with CI guard coverage banning the raw `ast_kernel` parser adapter. |
| Q-069 | DONE | Route parser block construction through refs. | `modules/parser.bkr` now builds blocks through `AstBlockRef`, appends statements through typed block helpers, and spans block nodes through the typed block span helper. |
| Q-070 | DONE | Route parser block consumers through refs. | Parser control-flow, match-block, and function-body paths now consume `parse_block_ref` internally while `parse_block` remains a raw compatibility wrapper. |
| Q-071 | DONE | Route parser statement consumers through refs. | Parser let/return/simple/if/for statement paths now expose typed `parse_*_stmt_ref` helpers, and block construction appends typed statements without raw statement-node roundtrips. |
| Q-072 | DONE | Route parser expression consumers through refs. | Parser expression consumers now use `parse_expr_ref`, with raw `parse_expr` retained only as the bootstrap compatibility adapter. |
...
| Q-080 | DONE | Route parser bitwise-and consumers through refs. | Parser bitwise-and parsing now exposes `parse_bit_and_ref`, and bitwise-xor parsing consumes typed bitwise-and refs without raw bitwise-and-node roundtrips. |
...
| Q-118 | DONE | Route AST-report if fields through typed refs. | Complete-tree if JSON now reads condition, then block, and optional else handle through typed statement/block accessors. |
...
| Q-206 | DONE | Replace if/while statement records with structs. | If/while constructors pack `AstIf`/`AstWhile`; typed unpack APIs remain; cond/body/else readers stay on `ast_field`; `tests/106_ast_if_struct.bkr` proves struct param/return/field use; CI rejects `ast_node_ref_new(NODE_IF)` / `NODE_WHILE`. |
...
| 2026-05-02 | Added typed AST constructor refs. | `modules/ast.bkr` now constructs type/expression/statement/item/block/kernel nodes through typed-returning helpers while preserving raw adapters; CI guards against direct raw constructor bodies returning. |
| 2026-05-02 | Added parser category span refs. | `modules/parser.bkr` can now attach spans to typed type/expression/statement/item/block refs directly, backed by AST category-to-node upcast helpers and CI guard coverage. |
| 2026-05-02 | Routed parser primary construction through typed refs. | Parser type nodes, match patterns, primary expressions, calls, literals, arrays, and unary expressions now use typed AST constructors before adapting back to the current raw parser return contract. |
| 2026-05-02 | Routed parser operator construction through typed refs. | Parser postfix, binary, ternary, assignment, and expression-statement nodes now use typed AST constructors and explicit typed span helpers before adapting back to the current raw parser return contract. |
| 2026-05-02 | Routed parser simple-statement construction through typed refs. | Parser let, return, break, and continue construction now uses typed statement constructors and typed parser span helpers before adapting back to raw parser returns. |
| 2026-05-02 | Routed parser control-flow construction through typed refs. | Parser if, for, while, and loop construction now uses typed statement constructors and typed parser span helpers; item and param construction remain raw pending a separate stage2-safe slice. |
| 2026-05-02 | Rolled parser param/field/item construction back to bootstrap raw adapters. | Post-merge `main` self-host smoke showed the Q-066 frontier could hang during full `bkrc` self-compilation even when PR checks passed; the stable parser construction frontier is now primary/operator/simple-statement/control-flow refs. |
| 2026-05-02 | Isolated parser kernel root typed conversion and kept it blocked. | PR #88 proved kernel root conversion could still time out in GitHub self-host smoke before C output joining was stabilized, so the kernel root stayed raw at that frontier. |
| 2026-05-02 | Stabilized full self-host smoke timeout budget. | The full `bkrc` stage1/stage2 smoke now has explicit phase checkpoints and a larger GitHub-only timeout budget so current self-host validation is not killed by runner variance after parser item routing. |
| 2026-05-02 | Added temp-dir self-host phase trace capture. | `self-host-compile` can now enable a Bunker-written phase trace with `BUNKER_SELF_HOST_TRACE=1`; CI live-prints the temp trace file while stage1 full `bkrc` compilation runs, avoiding local builds while diagnosing the remaining runner-killed hang. |
| 2026-05-02 | Added linear `join_lines` runtime helper for self-host C generation. | CI trace isolated the runner kill to `gen_c_program`; replacing the quadratic final C line-join with a Rust JIT and C runtime builtin should reduce peak memory and time for full `bkrc` self-compilation. |
| 2026-05-02 | Re-routed parser param/field construction through typed refs after codegen stabilization. | With full self-host smoke stabilized by linear C output joining, Q-066 moved back to typed `AstNodeRef` construction while item and kernel roots moved into separate follow-up slices. |
| 2026-05-02 | Routed parser item construction through typed refs after Q-066 validation. | Function, const, and struct item roots now use typed `AstItemRef` constructors and item span helpers, preparing the final Q-068 kernel-root conversion. |
| 2026-05-02 | Routed parser kernel root construction through typed refs after Q-067 validation. | `parse_kernel_ref` now converts item/name tables to typed collection refs and builds the root with `ast_kernel_ref`, closing the parser construction frontier. |
| 2026-05-02 | Routed parser block construction through typed refs. | `parse_block` now owns an `AstBlockRef`, appends typed statement refs, and returns through the typed block span helper instead of raw block adapters. |
| 2026-05-02 | Routed parser block consumers through typed refs. | Internal parser consumers now call `parse_block_ref` for function bodies, match-arm blocks, and control-flow bodies, leaving `parse_block` as a compatibility adapter only. |
| 2026-05-02 | Routed parser statement consumers through typed refs. | Let, return, simple, if, and for statement parsing now expose `AstStmtRef` internals while raw statement-returning functions stay as compatibility wrappers. |
| 2026-05-02 | Routed parser expression consumers through typed refs. | Parser match arms, expression lists, statement conditions, optional expression handles, and const initializers now consume `parse_expr_ref`; CI guards raw `parse_expr` consumer regressions. |
...
| 2026-05-02 | Routed parser bitwise-and consumers through typed refs. | `parse_bit_and_ref` now owns `&` expression construction, and `parse_bit_xor` consumes bitwise-and expressions through `AstExprRef` operands. |
...
| 2026-05-08 | Routed AST-report if fields through typed refs. | Complete-tree if JSON now reads control-flow child handles through `AstStmtRef` and `AstBlockRef` accessors. |
...
| 2026-08-13 | Replaced if/while statement records with structs. | If/while constructors pack `AstIf`/`AstWhile`; fixture `106_ast_if_struct.bkr` covers struct param/return/field use. |
