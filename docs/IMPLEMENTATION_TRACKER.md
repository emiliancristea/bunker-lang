...
| Q-068 | DONE | Route parser kernel construction through refs. | `modules/parser.bkr` now builds the kernel root through typed node/list refs with explicit typed locals, completing parser-side AST construction routing for the current bootstrap parser return contract. |
...
| 2026-05-02 | Routed parser kernel construction through typed refs. | Parser kernel root construction now uses typed node/list refs with explicit typed locals before adapting back to raw parser returns. |
